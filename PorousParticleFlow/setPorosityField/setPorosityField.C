...
int main(int argc, char *argv[])
{
    ...
    (
        "dict",
        "file",
        "Alternative setPorosityFieldDict dictionary"
    );
    ...
    const word dictName("setPorosityFieldDict");
    #include "setSystemMeshDictionaryIO.H"

    IOdictionary setPorosityFieldDict(dictIO);

    Info<< "Reading " << setPorosityFieldDict.name() << endl;

    const word fieldName = setPorosityFieldDict.get<word>("field");
    const scalar value = setPorosityFieldDict.get<scalar>("value");
    const bool invert = setPorosityFieldDict.getOrDefault("invert", false);
    const bool writeOBJ = setPorosityFieldDict.getOrDefault("writeOBJ", false);
    ...
    (
        setPorosityFieldDict.get<word>("type"),
        setPorosityFieldDict
    );
    ...
    setPorosity(alpha1, facePts, f, writeOBJ, value);
